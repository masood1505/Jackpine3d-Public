...
/**
 * SpatiaLite spatial DBMS dialect information for 3D queries.
 */
public class SpatialSqlDialectForSpatialLite3D implements SpatialSqlDialect3D {
    static String SRID = "";

    static {
        try (InputStream input = SpatialSqlDialectForSpatialLite3D.class.getClassLoader()
            ...
            props.load(input);
            // Change this to use SPATIALITE_SRID property instead of SQLSERVER_SRID
            SRID = props.getProperty("SPATIALITE_SRID", "4326").trim(); // Default to WGS84 if not specified
        } catch (Exception e) {
    ...
    public SupportedSqlDialect getSqlDialectType() {
        // You'll need to add SpatiaLite to your SupportedSqlDialect enum
        // For now, this assumes you have or will add it
        return SupportedSqlDialect.SpatiaLite;
        // If SpatiaLite doesn't exist in the enum yet, you might temporarily use:
        // return SupportedSqlDialect.SQLite; // or whatever closest option exists
    }

    // SpatiaLite uses different spatial function syntax than SQL Server
    public String getSelectBuffer3D() {
        // SpatiaLite uses ST_Buffer function, not STBuffer method
        return "SELECT ST_Buffer(a.geom, 5280) FROM arealm_merge a";
    }
    
    // Additional SpatiaLite-specific spatial functions you might need

    public String getSelectIntersection3D() {
        return "SELECT ST_Intersection(a.geom, b.geom) FROM arealm_merge a, arealm_merge b";
    }
    
    public String getSelectWithin3D() {
        return "SELECT * FROM arealm_merge a WHERE ST_Within(a.geom, ?)";
    }
    
    // SpatiaLite geometry creation functions
    public String createPoint3D(double x, double y, double z) {
        return String.format("ST_GeomFromText('POINT Z(%f %f %f)', %s)", x, y, z, SRID);
    }
    
    public String createPolygon3D(String wktPolygon) {
        return String.format("ST_GeomFromText('%s', %s)", wktPolygon, SRID);
    }
