...
public class ReadSpatialClosestPoint3D extends SpatialScenarioBase {

    private static final Logger logger = Logger.getLogger(ReadSpatialClosestPoint3D.class);
    private PreparedStatement[] pstmtArray;
    private Connection conn;
    private SpatialTableHelper3D helper3D;

    public ReadSpatialClosestPoint3D(Properties props) {
    ...
    public void prepare() throws Exception {
        ...
        String sql = getSelectLineClosestPoint3D();
        ...
        try {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    String closestPointGeom = rs.getString("closest_point_geom");
                    
                    logger.info("Closest point found: California ID: " + californiaId + 
                                ", California GML ID: " + californiaGmlId + 
                                ", Closest Point Geometry: " + closestPointGeom);
                }
            }
    ...
    private String getSelectLineClosestPoint3D() {
        return "SELECT \n" +
